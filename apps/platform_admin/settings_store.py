...
SPECS: tuple[SettingSpec, ...] = (
    # ---------------------------------------------------------- invoicing
    SettingSpec(
        "appearance.illustration_style", SettingKind.STRING, "appearance",
        "Illustration style",
    ...
    # ---------------------------------------------------------- invoicing
    SettingSpec(
        "invoice.issuer_name", SettingKind.STRING, "invoicing",
        "Issuer name", "The legal entity that issues invoices.",
        default="LedgerFlow",
    ),
    SettingSpec(
        "invoice.issuer_address", SettingKind.STRING, "invoicing",
        "Issuer address", "Printed on every invoice.", default="",
    ),
    SettingSpec(
        "invoice.issuer_email", SettingKind.STRING, "invoicing",
        "Billing contact", "Where invoice replies go.",
        default="billing@ledgerflow.app",
    ),
    SettingSpec(
        "invoice.issuer_tax_id", SettingKind.STRING, "invoicing",
        "Tax ID", "VAT/GST registration number, if you have one.", default="",
    ),
    SettingSpec(
        "invoice.default_tax_rate_bps", SettingKind.INTEGER, "invoicing",
        "Default tax rate (basis points)",
        "2000 = 20%. Applied to new invoices unless overridden.", default=0,
    ),
    SettingSpec(
        "invoice.tax_label", SettingKind.STRING, "invoicing",
        "Tax label", 'What to call it on the document — "VAT", "GST", "Sales tax".',
        default="Tax",
    ),
    SettingSpec(
        "invoice.payment_terms_days", SettingKind.INTEGER, "invoicing",
        "Payment terms (days)", "Days from issue to due date.", default=14,
    ),
    # ------------------------------------------------------------ payments
    SettingSpec(
        "payments.stripe_enabled", SettingKind.BOOLEAN, "payments",
        "Offer Stripe", "Whether customers can pay by card.", default=True,
    ),
    SettingSpec(
        "payments.mpesa_enabled", SettingKind.BOOLEAN, "payments",
        "Offer M-PESA", "Whether customers can pay by M-PESA.", default=True,
    ),
    SettingSpec(
        "payments.stripe_secret_key", SettingKind.SECRET, "payments",
        "Stripe secret key",
        "Overrides STRIPE_SECRET_KEY. Prefer the environment; set here only if "
        "you need to rotate without a deploy.",
        env_setting="STRIPE_SECRET_KEY", write_only=True,
    ),
    SettingSpec(
        "payments.stripe_webhook_secret", SettingKind.SECRET, "payments",
        "Stripe webhook secret", "Overrides STRIPE_WEBHOOK_SECRET.",
        env_setting="STRIPE_WEBHOOK_SECRET", write_only=True,
    ),
    SettingSpec(
        "payments.mpesa_consumer_key", SettingKind.SECRET, "payments",
        "M-PESA consumer key", "Overrides MPESA_CONSUMER_KEY.",
        env_setting="MPESA_CONSUMER_KEY", write_only=True,
    ),
    SettingSpec(
        "payments.mpesa_consumer_secret", SettingKind.SECRET, "payments",
        "M-PESA consumer secret", "Overrides MPESA_CONSUMER_SECRET.",
        env_setting="MPESA_CONSUMER_SECRET", write_only=True,
    ),
    SettingSpec(
        "payments.mpesa_shortcode", SettingKind.STRING, "payments",
        "M-PESA shortcode", "Overrides MPESA_SHORTCODE.",
        env_setting="MPESA_SHORTCODE", default="",
    ),
    # ------------------------------------------------------------------ ai
    SettingSpec(
        "ai.enabled", SettingKind.BOOLEAN, "ai",
        "AI features available",
        "Master switch. Off means no workspace gets AI, whatever their plan.",
        env_setting="LLM_ENABLED", default=False,
    ),
    SettingSpec(
        "ai.provider", SettingKind.STRING, "ai",
        "Provider", "google, groq, openai, anthropic, ollama, …",
        env_setting="LLM_PROVIDER", default="",
    ),
    SettingSpec(
        "ai.model", SettingKind.STRING, "ai",
        "Model", "Model identifier for the chosen provider.",
        env_setting="LLM_MODEL", default="",
    ),
    SettingSpec(
        "ai.api_key", SettingKind.SECRET, "ai",
        "API key", "Overrides LLM_API_KEY.",
        env_setting="LLM_API_KEY", write_only=True,
    ),
    SettingSpec(
        "ai.share_financial_context", SettingKind.BOOLEAN, "ai",
        ...
        env_setting="LLM_SHARE_FINANCIAL_CONTEXT", default=False,
    ),
    # ----------------------------------------------------------- operations
    SettingSpec(
        "ops.queue_backlog_threshold", SettingKind.INTEGER, "operations",
        "Queue backlog alert threshold",
        "Depth above which the health dashboard reports workers as degraded.",
        env_setting="PLATFORM_QUEUE_BACKLOG_THRESHOLD", default=500,
    ),
    SettingSpec(
        "ops.impersonation_ttl_minutes", SettingKind.INTEGER, "operations",
        "Impersonation session length (minutes)",
        "How long a support session stays usable before expiring on its own.",
        env_setting="PLATFORM_IMPERSONATION_TTL_MINUTES", default=30,
    ),
)
