...
import { Badge, Banner, Button, ConfirmAction, Heading, IconButton, Input, Modal, PageHeader, Select, Stack, Table, Text } from "../ui";
...
export function MembersPage() {
  ...
  const memberColumns: Column<Member>[] = [
    ...
    { key: "joined", header: "Joined", render: (m) => <span className="lf-cell-meta">{formatDateLong(m.created_at)}</span> },
    {
      key: "actions",
      header: "",
      align: "right",
      render: (m) =>
        canManage && m.user_id !== user?.id ? (
          <ConfirmAction
            ...
            onConfirm={() => removeMember.mutateAsync(m.id)}
          />
        ) : null,
    },
  ];
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: "var(--lf-space-8)" }}>
        <Table columns={memberColumns} rows={members ?? []} rowKey={(m) => m.id} responsive={false} caption="Members" />
      </div>
